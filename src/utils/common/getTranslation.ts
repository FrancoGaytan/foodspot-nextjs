...
export async function getTranslation<T = TranslationNamespace>(namespace: string, routeLang?: string): Promise<TranslationResult<T>> {
  const lang = routeLang ?? await getLocaleFromCookieServer();
