...
- Add event chat. The prototype presents chat as a future product capability, but the current backend exposes no message model, event chat endpoint, real-time transport, retention policy, or authorization contract. The Next.js app must not show a sendable chat interface until those contracts exist.
